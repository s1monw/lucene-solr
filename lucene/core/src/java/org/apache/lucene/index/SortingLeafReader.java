...
class SortingLeafReader extends FilterLeafReader {
  ...
  static LeafReader wrap(LeafReader reader, Sorter.DocMap docMap, Sort sort) {
    ...
    return new SortingLeafReader(reader, docMap, sort);
  }

  final Sorter.DocMap docMap; // pkg-protected to avoid synthetic accessor methods
  final LeafMetaData metaData;

  private SortingLeafReader(final LeafReader in, final Sorter.DocMap docMap, Sort indexSort) {
    super(in);
    this.docMap = docMap;
    LeafMetaData metaData = in.getMetaData();
    this.metaData = new LeafMetaData(metaData.getCreatedVersionMajor(), metaData.getMinVersion(), indexSort);
  }
